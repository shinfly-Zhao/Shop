...
class BaseChat(models.Model):
    ...
    user = models.ForeignKey(UserProfile, verbose_name="所属用户", on_delete=models.SET_NULL, related_name="base_chat",null=True,blank=True)
    is_top = models.CharField(max_length=10, choices=(("top", "置顶"), ("basic", "不置顶")), default="basic",
                              verbose_name="是否置顶")
    is_use = models.BooleanField(default=True, verbose_name="是否通过")
    is_jb = models.BooleanField(default=False, verbose_name="是否举报")
    jb_time = models.DateTimeField(null=True,blank=True, verbose_name="发布时间")
    ...
    def get_twonums(self):
        return self.rreplybaschat_set.filter(chat=self).count()

    get_twonums.short_description = u"二级评论"
